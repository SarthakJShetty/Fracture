...
```python webscraper --search "Gears" --num_images 100 --directory /path/to/dataset/directory```
...
```python retrain.py --image_dir path/to/dataset/directory --path="project_name"```
...
5. We can now use ```label_image.py``` to identify the whether the given component is defective or not. Usage:
```python label_image.py --graph=path/of/tmp/file/genrated/output_graph.pb --labels=path/of/tmp/file/project_name/genrated/labels.txt --output_layer=final_result```
